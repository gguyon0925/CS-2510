...
class ExamplesMaze {
  ...
  // Test helper method to count the number of connected components in the maze
int countConnectedComponents(Maze maze) {
  Set<Cell> visited = new HashSet<>();
  int count = 0;

  for (ArrayList<Cell> row : maze.cellCol) {
    for (Cell cell : row) {
      if (!visited.contains(cell)) {
        count++;
        dfsConnectedComponents(maze, cell, visited);
      }
    }
  }
  return count;
...
// DFS traversal to mark connected components in the maze
void dfsConnectedComponents(Maze maze, Cell cell, Set<Cell> visited) {
  visited.add(cell);
  for (Edge edge : maze.edgeList) {
    if (edge.first.equals(cell) || edge.second.equals(cell)) {
      Cell neighbor = (edge.first.equals(cell)) ? edge.second : edge.first;
      if (!visited.contains(neighbor)) {
        dfsConnectedComponents(maze, neighbor, visited);
      }
    }
  }
}
  ...
  void testVerticalEdge(Tester t) {
    ...
    t.checkExpect(edge3.verticalEdge(), true);
  }
  ...
  void testDrawCell(Tester t) {
    ...
    cell1.visit();
    t.checkExpect(cell1.drawCell(), new RectangleImage(20, 20, "solid", Color.cyan));
    cell1.unvisit();
    t.checkExpect(cell1.drawCell(), new RectangleImage(20, 20, "solid", Color.white));
  }
  ...
  void testBuildMaze(Tester t) {
    ...
    t.checkExpect(maze2.cellCol.size(), 5);
    t.checkExpect(maze2.cellRow.size(), 5);
    t.checkExpect(maze2.edgeList.size(), 16);
    t.checkExpect(maze2.cellMap.size(), 25);
    maze1.buildMaze();
    maze2.buildMaze();

    t.checkExpect(maze1.cellCol.size(), 20);
    t.checkExpect(maze1.cellCol.get(0).size(), 10);
    t.checkExpect(maze1.cellCol.get(9).get(9).col, Color.red);

    t.checkExpect(maze2.cellCol.size(), 10);
    t.checkExpect(maze2.cellCol.get(0).size(), 5);
    t.checkExpect(maze2.cellCol.get(4).get(4).col, Color.red);
  }
  ...
  void testFind(Tester t) {
    initData();
    maze1.buildMaze();
    t.checkExpect(maze1.find(maze1.cellCol.get(0).get(0)), maze1.cellCol.get(0).get(0));
    t.checkExpect(maze1.find(maze1.cellCol.get(9).get(9)), maze1.cellCol.get(9).get(9));
  ...
  void testDrawEdges(Tester t) {
    initData();
    // Test drawEdges method when edge is vertical
    WorldScene scene1 = new WorldScene(200, 200);
    maze1.drawEdges(scene1);
    t.checkExpect(imageExistsAtPosn(scene1, new Posn(edge1.first.x * 20, edge1.first.y * 20),
        new LineImage(new Posn(0, 20), Color.black).movePinhole(0, -30)), true);

    // Test drawEdges method when edge is not vertical (horizontal)
    WorldScene scene2 = new WorldScene(200, 200);
    maze1.drawEdges(scene2);
    t.checkExpect(imageExistsAtPosn(scene2, new Posn(edge2.first.x * 20, edge2.first.y * 20),
        new LineImage(new Posn(20, 0), Color.black).movePinhole(-30, 0)), true);

    // Test drawEdges method when edge is self-loop (edge from a cell to itself)
    WorldScene scene3 = new WorldScene(200, 200);
    maze1.drawEdges(scene3);
    t.checkExpect(imageExistsAtPosn(scene3, new Posn(edge3.first.x * 20, edge3.first.y * 20),
        new LineImage(new Posn(0, 20), Color.black).movePinhole(0, -30)), true);
  }
  ...
  // FOLLOWING 3 METHODS ARE NOT NEEDED FOR PT.1
